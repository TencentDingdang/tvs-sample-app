...
import com.tencent.dingdang.tvs.AudioPlayerStateMachine.AudioPlayerState;
...
import com.tencent.dingdang.tvs.message.response.audioplayer.AudioItem;
...
import com.tencent.dingdang.tvs.message.response.audioplayer.Stream;
...
public class TVSAudioPlayer {

    private static final Logger log = LoggerFactory.getLogger(TVSAudioPlayer.class);
    ...
    // Cache of URLs associated with the current TVSPlayItem/stream
    ...
    // VLCJ volumes are between 0-200. volumes are from 0-100. These constants are used to
    ...
    private final Set<TVSSpeechListener> listeners;
    ...
    public TVSAudioPlayer(TVSController controller) {
    ...
    public void registerTVSSpeechListener(TVSSpeechListener listener) {
     ...
     */
    public void interruptAllTVSOutput() {
        log.debug("Interrupting all output");
        if (isSpeaking()) {
     ...
     */
    public void resumeAllTVSOutput() {
        log.debug("Resuming all output");
        ...
        Thread thread = new Thread() {
            ...
            public void run() {
                ...
                if (isSpeaking() || isAlarming()) {
                    // pause if speaking or there is an active alert.
                    interruptContent();
                }
            }
        };
    ...
    public void startAlert() {
        if (!isAlarming()) {
            interruptContent();
            if (isSpeaking()) {
                // alerts are in the background when speaking
                alertState = AlertState.INTERRUPTED;
            } else {
    ...
    private void notifyTVSSpeechStarted() {
        for (TVSSpeechListener listener : listeners) {
            listener.onTVSSpeechStarted();
        }
    }

    private void notifyTVSSpeechFinished() {
        for (TVSSpeechListener listener : listeners) {
            listener.onTVSSpeechFinished();
        }
    }
    ...
    public interface TVSSpeechListener {
        void onTVSSpeechStarted();

        void onTVSSpeechFinished();
    }
