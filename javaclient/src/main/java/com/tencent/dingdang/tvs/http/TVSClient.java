...
import com.tencent.dingdang.tvs.TVSRequest;
...
import com.tencent.dingdang.tvs.exception.TVSException;
...
import com.tencent.dingdang.tvs.exception.TVSSystemExceptionCode;
import com.tencent.dingdang.tvs.http.MultipartParser.MultipartParserConsumer;
import com.tencent.dingdang.tvs.http.jetty.InputStreamResponseListener;
import com.tencent.dingdang.tvs.http.jetty.PingSendingHttpClientTransportOverHTTP2;
import com.tencent.dingdang.tvs.http.jetty.PingSendingHttpClientTransportOverHTTP2.ConnectionListener;
import com.tencent.dingdang.tvs.message.Message;
import com.tencent.dingdang.tvs.message.request.RequestBody;
import com.tencent.dingdang.tvs.message.response.TVSExceptionResponse;
...
public class TVSClient implements ConnectionListener {
    private static final Logger log = LoggerFactory.getLogger(TVSClient.class);
    ...
    private final BlockingQueue<TVSRequest> requestQueue;
     ...
     * @throws Exception
     */
    public TVSClient(URL host, MultipartParserConsumer multipartParserConsumer,
     ...
     * @param tvsRequest
     */
    private void doRequest(TVSRequest tvsRequest) {

        Callable<Void> task = new Callable<Void>() {
            @Override
            public Void call() throws Exception {
                Request request =
                        createRequest(tvsRequest.getResource(), tvsRequest.getContentProvider());
                doRequestActual(request, tvsRequest.getRequestListener(),
                        tvsRequest.getMultipartParser());
                return null;
            }
        };

        try {
            tvsRequest.getRetryPolicy().tryCall(task, RequestException.class);
            ...
            tvsRequest.getRequestListener().ifPresent(l -> l.onRequestError(e));
        }
    }
    ...
    private void doRequestActual(Request request, Optional<RequestListener> requestListener,
            MultipartParser multipartParser) throws TVSException, IOException {
        ...
        } catch (TVSJsonProcessingException e) {
    ...
    public void parseException(InputStream inputStream, MessageParser parser)
            throws IOException, TVSSystemException {
        ByteArrayOutputStream data = new ByteArrayOutputStream();
        IOUtils.copy(inputStream, data);
        Message message = parser.parseServerMessage(data.toByteArray());
        if (message instanceof TVSExceptionResponse) {
            ((TVSExceptionResponse) message).throwException();
        }
    }
    ...
    public void sendEvent(RequestBody body, RequestListener listener)
        ...
        enqueueRequest(new TVSRequest(Resource.EVENTS, multipartContent,
    ...
    private void enqueueRequest(TVSRequest request) {
    ...
    /**
     * Set the access token to use for all requests to TVS.
     *
     * @param accessToken
     */

    private static void setAccessTokenValue(String accessToken){
        TVSClient.accessToken = accessToken;
    }
    ...
    private class DownchannelRequestThread extends Thread {
        ...
        private void openConnection() {
            while (running) {
                ...
                TVSRequest tvsRequest = new TVSRequest(Resource.DIRECTIVES, null,
                        ...
                        downchannelParser, new RequestListener() {
                            ...
                            private boolean shouldExceptionCauseShutdown(Throwable e) {
                                return (e instanceof TVSSystemException)
                                        && (TVSSystemExceptionCode.UNAUTHORIZED_REQUEST_EXCEPTION == ((TVSSystemException) e)
                                                .getExceptionCode());
                            }

                        });

                doRequest(tvsRequest);
    ...
    private class RequestThread extends Thread {
        private BlockingQueue<TVSRequest> queue;

        public RequestThread(BlockingQueue<TVSRequest> queue) {
            this.queue = queue;
            setName(this.getClass().getSimpleName());
        }
